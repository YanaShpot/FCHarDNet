...
class multiResolutionFusion(nn.Module):
    ...
    def forward(self, x_high, x_low):
        high_upsampled = F.upsample(
            self.conv_high(x_high), scale_factor=self.up_scale_high, mode="bilinear", align_corners=True
        )
        ...
        low_upsampled = F.upsample(
            self.conv_low(x_low), scale_factor=self.up_scale_low, mode="bilinear", align_corners=True
        )
...
class pyramidPooling(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training or self.model_name != "icnet":  # general settings or pspnet
            ...
            # harcoded original values
            # k_sizes = [(8, 15), (13, 25), (17, 33), (33, 65)]
            # strides = [(5, 10), (10, 20), (16, 32), (33, 65)]

            #Option for diff sizes of input images(?)
            k_sizes = [] 
            strides = [] 
            for pool_size in self.pool_sizes: 
                k_sizes.append((int(h/pool_size), int(w/pool_size))) 
